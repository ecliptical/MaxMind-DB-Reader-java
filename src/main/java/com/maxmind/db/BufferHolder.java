...
final class BufferHolder {
	...
	/**
	 * Construct a ThreadBuffer from the provided InputStream.
	 *
	 * @param stream the source of my bytes.
	 ...
	 * @throws NullPointerException if you provide a NULL InputStream
	 */
	BufferHolder(InputStream stream, int length) throws IOException {
		if (null == stream) {
			throw new NullPointerException("Unable to use a NULL InputStream");
		}
		this.buffer = ByteBuffer.allocate(length);
		while (this.buffer.hasRemaining()) {
			int c = stream.read(this.buffer.array(), this.buffer.arrayOffset() + this.buffer.position(), this.buffer.remaining());
			if (c == -1)
				throw new EOFException();

			this.buffer.position(this.buffer.position() + c);
		}
		this.buffer.flip();
	}
