...
public final class Reader implements Closeable {
	 ...
	 *
	 * @param source the InputStream that contains the MaxMind DB file.
	 * @param source the length (in bytes) of the MaxMind DB file.
	 * @throws IOException if there is an error reading from the Stream.
	 */
	public Reader(InputStream source, int length) throws IOException {
		this(new BufferHolder(source, length), "<InputStream>");
	}
